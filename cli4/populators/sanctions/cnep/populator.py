"""
CLI4 CNEP Populator
Populate vendor_sanctions table with Portal da Transparência CNEP data
...
class CNEPPopulator:
    """Populate vendor sanctions table with Portal da Transparência CNEP data"""
    ...
    def populate(self, max_pages: Optional[int] = None,
        ...
        print("⚠️  CNEP SANCTIONS POPULATION")
        print("=" * 60)
        print("Portal da Transparência CNEP sanctions for corruption detection")
        print()
    ...
    def _fetch_sanctions_page(self, page: int) -> List[Dict]:
        ...
        try:
            ...
            response = self.portal_client.session.get(
                f"{self.portal_client.base_url}cnep",
                params={'pagina': page},
                timeout=30
            )
    ...
    def _build_sanction_record(self, sanction: Dict) -> Optional[Dict]:
        """Build sanction record from API data following our database schema"""
        try:
            # Extract entity info
            pessoa = sanction.get('pessoa', {})
            sancionado = sanction.get('sancionado', {})
            orgao_sancionador = sanction.get('orgaoSancionador', {})
            tipo_sancao = sanction.get('tipoSancao', {})

            # Get CNPJ from pessoa or sancionado
            cnpj_raw = pessoa.get('cnpjFormatado', '') or sancionado.get('codigoFormatado', '')
            if not cnpj_raw:
            ...
            # Parse dates
            start_date = self._parse_date(sanction.get('dataInicioSancao'))
            end_date = self._parse_date(sanction.get('dataFimSancao'))

            # Extract penalty amount from CNEP valorMulta field
            penalty_amount = None
            valor_multa = sanction.get('valorMulta')
            if valor_multa and isinstance(valor_multa, (int, float)):
                penalty_amount = float(valor_multa)

            record = {
                'cnpj_cpf': cnpj_clean,
                'entity_name': self._normalize_text(
                    sancionado.get('nome') or pessoa.get('nome'), 500
                ),
                'sanction_type': self._normalize_text(
                    tipo_sancao.get('descricaoResumida'), 100
                ),
                'sanction_description': self._normalize_text(
                    tipo_sancao.get('descricaoPortal'), 2000
                ),
                'sanction_start_date': start_date,
                'sanction_end_date': end_date,
                'sanctioning_agency': self._normalize_text(
                    orgao_sancionador.get('nome'), 255
                ),
                'sanctioning_state': self._normalize_text(
                    orgao_sancionador.get('siglaUf'), 10
                ),
                'sanctioning_process': self._normalize_text(
                    sanction.get('numeroProcesso'), 100
                ),
                'penalty_amount': penalty_amount,
                'is_active': self._is_sanction_active(start_date, end_date),
                'data_source': 'PORTAL_TRANSPARENCIA_CNEP',
                'api_reference_id': str(sanction.get('id', '')),
            ...
            print(f"        ⚠️ Error building sanction record: {e}")
            return None
...
def main():
    ...
    populator = CNEPPopulator(logger, rate_limiter)
