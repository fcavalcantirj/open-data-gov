"""
CLI4 Sanctions Populator
Populate vendor_sanctions table with Portal da Transparência CEIS data
...
class SanctionsPopulator:
    """Populate vendor sanctions table with Portal da Transparência data"""
    ...
    def populate(self, max_pages: Optional[int] = None,
        ...
        DependencyChecker.print_dependency_warning(
            ...
            current_step="SANCTIONS POPULATION"
        )
        ...
        print(f"\\n✅ Sanctions population completed")
    ...
    def _fetch_sanctions_page(self, page: int) -> List[Dict]:
        ...
        try:
            ...
            response = self.portal_client.session.get(
                f"{self.portal_client.base_url}ceis",
                params={'pagina': page},
                timeout=30
            )
    ...
    def _build_sanction_record(self, sanction: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract penalty amount if available
            penalty_amount = None
            # Note: Portal API doesn't seem to include monetary amounts in basic CEIS data
...
def main():
    ...
    populator = SanctionsPopulator(logger, rate_limiter)
